...
# --- タイムバケツ（やりたいこと × 年代）------------------------------------
# 夫婦で共有する「タイムバケツ」アイテムを保存する専用シート（行ベース）。
# ここに配置した金額は、ライフプランの「特別支出」として代表年齢の年に計上される。
TIME_BUCKET_SHEET = "タイムバケツ"
TIME_BUCKET_HEADER = ["ID", "タイトル", "予定金額", "年代", "実行年齢"]

# 年代バケツ（列）。先頭の「未分類」は ToDo 置き場で、支出には計上しない。
TIME_BUCKETS = ["未分類", "30代", "40代", "50代", "60代", "70代以上"]

# 年代 → その年代の支出を計上する「夫（本人）の代表年齢」（中央値）。
# アイテムに実行年齢が個別指定されていればそちらを優先する。
BUCKET_REPRESENTATIVE_AGE = {
    "30代": 35,
    "40代": 45,
    "50代": 55,
    "60代": 65,
...
# 特別支出（タイムバケツ由来）のキャッシュフロー列名。
COL_SPECIAL_EXPENSE = "特別支出"
